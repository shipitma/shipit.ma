...
export const HelpMenu: FC<HelpMenuProps> = ({ align = "end", className = "" }) => {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent className="w-72" align={align} forceMount>
        <DropdownMenuItem asChild>
          <a href="tel:0522300900" className={`flex items-center gap-2 py-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
            <Phone className="h-5 w-5 text-gray-500" />
            <span>{t('mainLayout.help.callUs', 'Appelez au')} <span className="font-semibold">0522 300 900</span></span>
          </a>
        </DropdownMenuItem>
        <DropdownMenuItem asChild>
          <a href="https://wa.me/212522300900" target="_blank" rel="noopener noreferrer" className={`flex items-center gap-2 py-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
            <span className="inline-flex items-center justify-center h-5 w-5 rounded bg-green-500">
            ...
            <span>{t('mainLayout.help.contactWhatsapp', 'Contactez sur WhatsApp')}</span>
          </a>
        </DropdownMenuItem>
        <DropdownMenuItem asChild>
          <a href="/help" className={`flex items-center gap-2 py-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
            <HelpCircle className="h-5 w-5 text-gray-500" />
            <span>{t('mainLayout.help.consultHelpPage', 'Consultez la page d\'aide')}</span>
          </a>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
